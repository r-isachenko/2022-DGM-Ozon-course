...
\begin{frame}{Wasserstein GAN with Gradient Penalty}
	\begin{block}{Theorem}
		...
		\begin{enumerate}
			...
			\]
			\item if $f^*$ is differentiable, $\gamma(\bx = \by) = 0$ and $\hat{\bx}_t = t \bx + (1 - t) \by$ with $\bx \sim \pi(\bx)$, $\by \sim p(\bx | \btheta)$, $t \in [0, 1]$ it holds that
			\[
	...
	How to compute $ \| \bW \|_2 = \lambda_{\text{max}}(\bW^T \bW)$? \\
	 ...
	 \begin{block}{Power iteration method}
	 	\begin{itemize}
	 		...
	 		\[
	 			\| \bW \|_2 = \lambda_{\text{max}}(\bW^T \bW) \approx \bu_{K}^T \bW \bv_{K}.
	 		\]
	 	\end{itemize}
	 \end{block}
