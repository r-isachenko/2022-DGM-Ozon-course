...
\begin{frame}{Stochastic differential equation (SDE)}
	\begin{block}{Theorem}
		\begin{align*}
			\btheta^* &= \argmin_{\btheta} \bbE_{\pi}\bigl\| \bs(\bx, \btheta) - \nabla_\bx \log \pi(\bx) \bigr\|^2_2 \\ &= \argmin_{\btheta} \bbE_{\pi}\Bigr[ \| \bs(\bx, \btheta) \|^2 + \text{trace}\bigl(\nabla_{\bx} \bs(\bx, \btheta)\bigr) \Bigr]
		\end{align*}
		Here $\nabla_{\bx} \bs(\bx, \btheta) = \nabla_{\bx}^2 \log p(\bx | \btheta)$.
	\end{block}
