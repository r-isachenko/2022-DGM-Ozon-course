...
\begin{frame}{Recap of previous lecture}
	...
	\begin{block}{Flow-based VAE prior}
		\vspace{-0.5cm}
		\[
			\log p(\bz | \blambda) = \log p(\bz^*) + \log \left| \det \left( \frac{d \bz^*}{d\bz} \right) \right| = \log p(\bepsilon) + \log \left| \det ( \bJ_g) \right| 
		\]
	\end{block}
...
\end{frame}
%=======
\begin{frame}{$\beta$-VAE objective}
...
\begin{frame}{DIP-VAE: disentangled posterior}
	...
	\begin{block}{DIP-VAE Objective}
		...
		{\footnotesize
			...
			= \frac{1}{n} \sum_{i=1}^n\left[ \bbE_{q(\bz | \bx_i)} \log p(\bx_i | \bz, \btheta) - KL(q(\bz | \bx_i) || p(\bz)) \right] -\lambda \cdot KL(q_{\text{agg}}(\bz) || p(\bz)) = \\
...
\end{frame}
%=======
\begin{frame}{DIP-VAE: moment matching}
		\vspace{-0.2cm}
		\[
			\cL_{\text{DIP}}(q, \btheta) = \frac{1}{n} \sum_{i=1}^n \cL_i(q, \btheta) -\lambda \cdot \underbrace{KL(q_{\text{agg}}(\bz) || p(\bz))}_{\text{intractable}}
		\]
...
\end{frame}
%=======
\section{Challenging disentanglement assumptions}
%=======
