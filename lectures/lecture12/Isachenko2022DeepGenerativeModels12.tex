...
%=======
\begin{frame}{Discrete VAE latents}
		Let $z \sim \text{Categorical}(\bpi)$, where 
		\[
			\bpi = (\pi_1, \dots, \pi_K), \quad \pi_k = P(z = z_k), \quad \sum_k \pi_k = 1.
		\]
		We assume that the prior distribution $p(z) = \text{Uniform}\{z_1, \dots, z_K\}$.
		\begin{multline*}
			KL(q(z| \bx, \bphi) || p(z)) = \sum_{k=1}^K q(z_k | \bx, \bphi) \log \frac{q(z_k | \bx, \bphi)}{p(z_k)} = 
			\\ = \sum_{k=1}^K q(z_k | \bx, \bphi) \left[\log q(z_k | \bx, \bphi)  - \log p(z_k) \right] = - H(q(z | \bx, \bphi)) + \log K. 
		\end{multline*}
			\begin{block}{ELBO}
				\vspace{-0.6cm}
				\[
					\mathcal{L} (\bphi, \btheta)  = \mathbb{E}_{q(z | \bx, \bphi)} \log p(\bx | z , \btheta) + H(q(z | \bx, \bphi)) - \log K \rightarrow \max_{\bphi, \btheta}.
				\]
				\vspace{-0.5cm}
			\end{block}
			\begin{itemize}
				\item Reparametrization trick does not work now.
...
\begin{frame}{Gumbel-softmax trick}
	...
	\begin{block}{Gumbel-max trick}
		...
		\]
		\vspace{-0.5cm} \\
		has a categorical distribution $z \sim \text{Categorical}(\bpi)$.
	\end{block}
	\begin{block}{Reparametrization trick}
		\vspace{-0.5cm}
		\[
			\nabla_{\bphi} \bbE_{q(z | \bphi) f(z)} = \bbE_{\text{Gumbel}(0, 1)} \nabla_{\bphi} f\bigl(\argmax_k [\log q(z_k | \bphi) + g_k]\bigr).
		\]
		\vspace{-0.5cm}
	\end{block}
...
\end{frame}
%=======
\begin{frame}{Concrete distribution}
	\vspace{-0.3cm}
...
\begin{frame}{Vector quantization}
	\begin{itemize}
		...
		\item Quantized representation $\bz_q \in \bbR^{W \times H \times C}$  for $\bz \in \bbR^C$ is defined by a nearest neighbour look-up using the shared dictionary space
		\[
...
\begin{frame}{Vector Quantized VAE}
	Let VAE latent variable $\hat{\bz} \in \bbR^{W \times H}$ is the discrete with spatial-independent variational posterior and prior distributions  
	\vspace{-0.3cm}
	\[
		q(\hat{\bz} | \bx, \bphi) = \prod_{i=1}^W \prod_{j=1}^H q(\hat{z}_{ij} | \bx, \bphi); \quad p(\hat{\bz}) = \prod_{i=1}^W \prod_{j=1}^H \text{Uniform}\{z_1, \dots, z_K\}.
	\]
	Let $\bz_e = \text{NN}(\bx, \bphi) \in \bbR^{W \times H \times C}$ is the encoder output.
	\[
		q(\hat{z}_{ij} = z_k^* | \bx, \bphi) = \begin{cases}
	...
	\vspace{-0.3cm}
	\begin{itemize}
		\item VAE posterior distribution $q(\hat{\bz} | \bx)$ is deterministic (zero entropy). 
		\item $KL(q(\hat{z}| \bx) || p(\hat{z}))$ term in ELBO is constant.
		\[
			KL(q(\hat{z} | \bx, \bphi) || p(z)) = - H(q(\hat{z} | \bx, \bphi)) + \log K = \log K. 
		\]
	\end{itemize}
